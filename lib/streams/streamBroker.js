module.exports = function() {
  ...
  var _removeWatcher = function(watcherClient) {
    var found = null;
    Object.keys(connectedBroadcasters).some(function(k){
  ...
  return {
    ...
    addBroadcaster: function(streamerId, userData) {
      ...
      if (connectedBroadcasters[userData.id]) {
        // Stream already exists - this may indicate that the stream was temporarily
        // disconnected and re-added.
        console.warn('Broadcaster connection already registered. Overriding');
        var oldStream = connectedBroadcasters[userData.id];
        var newStream = new Stream(streamerId, userData);
        newStream.watchers = oldStream.watchers;
        newStream.watchersIds = oldStream.watchersIds;
        newStream.streaming = oldStream.streaming;
        connectedBroadcasters[userData.id] = newStream;
      } else {
        connectedBroadcasters[userData.id] = new Stream(streamerId, userData);
      }
    },

    removeBroadcaster: function(broadcasterId, socketId) {
      if (broadcasterId in connectedBroadcasters && connectedBroadcasters[broadcasterId].streamId.id == socketId) {
