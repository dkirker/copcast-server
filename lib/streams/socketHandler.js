...
module.exports = function(io,auth, streams) {
  ...
  var handleAndroid = function(client, user) {
    ...
    console.log("welcome android: "+user.username + '('+client.id+')');
    ...
    client.on('disconnect', function() {
      mapUserIdToDisconnectTimeout[user.id] = setTimeout(function() {
        ...
        if (broker.removeBroadcaster(user.id, client.id)) {
          Object.keys(adminList).forEach(function (uid) {
    ...
    client.on('stopStreamingRequest', function() {
      console.log('android cancel streaming request : '+user.username);
      Object.keys(adminList).forEach(function(uid) {
        console.log('emitting cancel to stream request: '+client.id);
        adminList[uid].emit('stopStreamingRequest', {'userId': user.id});
      });
    });
    ...
    client.on('streamStopped', function() {
      Object.keys(adminList).forEach(function(uid) {
        ...
        adminList[uid].emit('streamStopped', {'userId': user.id});
      });
    });

    client.on('streamStarted', function() {
      Object.keys(adminList).forEach(function(uid) {
        console.log('emitting mission started: '+adminList[uid] + ' from mobile '+user.id);
        adminList[uid].emit('streamStarted', {'userId': user.id});
      });
    });
  ...
  var handleAdmin = function(client, user) {
    ...
    var unwatch = function() {
      ...
      if (bro != null && !broker.hasMoreWatchers(bro)) {
        console.log('no more watchers for: '+bro);
    ...
    client.on('unwatch', function() {
      console.log(user.username+ " unwatch ("+client.id+")");
      unwatch();
