...
app.get('/users/roles', auth.ensureAdmin, function (req, res) {
    var roles = null;
    if (req.user.role === 'admin_3'){
      roles = ['admin_3', 'admin_2', 'admin_1', 'mobile'];
    } else if (req.user.role === 'admin_2') {
      roles = ['admin_3', 'admin_2', 'admin_1', 'mobile'];
    } else if (req.user.role === 'admin_1') {
      roles = ['admin_3', 'admin_2', 'admin_1', 'mobile'];
    }
    res.send(roles);
});
...
app.get('/users', auth.ensureAdmin, function (req, res) {
  db.group.findById(req.user.groupId).then(function(group){
      var where = [],
          ...
          select = ['SELECT DISTINCT U.id, U.username, U.\"lastLocationUpdateDate\", U.role, U.name, U.\"profilePicture\", G.name as group FROM "users" U LEFT OUTER JOIN "groups" G ON U."groupId" = G.id'];
      ...
      db.sequelize.query(query, { logging : console.log, raw : true, replacements : params,
...
app.get('/users/:id/videos/:video.:format', auth.ensureAdmin, function (req, res) {
