...
app.post('/users/:email/reset_password', function (req, res) {
  db.user.findOne({where: {email: req.params.email}}).then(function (user) {
    ...
    var accessUrl = config.clientUrl +"reset_password/"+randomString
    sendEmail(req.params.email, accessUrl, user.username, function (success, errorMsg) {
      if (!success) {
...
function sendEmail(email, token, username, callback) {
