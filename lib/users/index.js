...
app.get('/users', auth.ensureAdminLevelOne, function (req, res) {
  db.group.findById(req.user.groupId).then(function (group) {
    var where = [],
      ...
      select = ['SELECT DISTINCT U.id, U.username, U.\"lastLocationUpdateDate\", U.name, U.\"profilePicture\", G.name as group, U.\"isEnabled\" FROM "users" U LEFT OUTER JOIN "groups" G ON U."groupId" = G.id'];
...
app.post('/users/:email/reset_password', function (req, res) {
  db.user.findOne({where: {email: req.params.email}}).then(function (user) {
    ...
    emailManager.sendEmail(req.params.email, accessUrl, user.username, function (success, errorMsg) {
      if (!success) {
