...
passport.use(new LocalStrategy({ passReqToCallback: true },
  function (req, username, password, done) {
    db.sequelize.query('SELECT * FROM users WHERE lower(username) = lower(?) LIMIT 1', {
      ...
      if (!users || !users.length) {
        ...
        users[0].validatePassword(password, function(valid) {
          if (!valid) {
            return done(null, false, {message: 'incorrect_password'});
          }

          var isAdmin = req.body.scope.indexOf('admin') > -1;
          if ( req.body.scope && isAdmin && !users[0].isAdmin ) {
          ...
          if (isAdmin) {
            db.history.registerAdminLoggin(users[0]);
          }
          done(null, users[0]);
          return null;
        });
      ...
      console.log(err);
      return done(null, false, {code: 500, message: 'server_error'});
    });
  }
));
