...
function getTotalRecordedHoursByUser(dateRange, userId, callback) {
  ...
  db.sequelize.query('SELECT SUM("duration") AS "sum" ' +
    ...
    'AND "user"."id" = :userId', {
    replacements: {userId: userId, fromDate: dateRange[0], toDate: dateRange[1]},
    type: db.sequelize.QueryTypes.SELECT
...
function loadUsersData(ret, where, fromDate, toDate, group, callback) {
  ret.users = [];
  db.user.findAll({
    where: mergeHashs(where, {lastLocationUpdateDate: {between: [fromDate, toDate]}}),
  }).then(function (users) {
    ret.activeOfficers = users.length;
    var finished = _.after(users.length * 2 * moment(toDate).diff(moment(fromDate), "days"), callback);
    if (users.length === 0) {
      callback();
    }
    for (var i = 0; i < users.length; i++) {
      ...
      for (var m = moment(fromDate); m.isBefore(toDate); m.add('days', 1)) {

        userObj.activities = [];

        getTotalRecordedHoursByUser([m.toDate(), m.hour(23).minute(59).seconds(59).toDate()], user.id,
