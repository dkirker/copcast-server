...
//streamed hours
  ...
  queryHistory = queryHistory + '	AND "hist"."date" BETWEEN :initialDate AND :finalDate ' +
    'ORDER BY "hist"."date"';
    ...
    for (var i = 0; i < users.length; i++) {
      ...
      for (var m = moment(fromDate.toString()); (m.isBefore(toDate, 'days') || m.isSame(toDate, 'days')); m.add('days', 1)) {
        ...
        getTotalRecordedHoursByUser(m.toDate(), moment(m.toString()).hour(23).minute(59).seconds(59).toDate(), user.id,
          function (date, userObj, total) {
            var activityObj = {date: date};
            if (total) {
              activityObj.recordedHours = total;
              userObj.activities.push(activityObj);
              getTotalOfState(moment(date).toDate(), moment(date).hour(23).minute(59).seconds(59).toDate(), group, user,
                'STREAMING', function (total) {
                this.streamedHours = total;
                finished();
              }.bind(activityObj));

              getTotalOfState(moment(date).toDate(), moment(date).hour(23).minute(59).seconds(59).toDate(), group, user,
                'PAUSED', function (total) {
