...
app.get('/report/use/:fromDate/:toDate', auth.ensureAdminLevelTwo, function (req, res) {
  ...
  db.group.findByPk(req.user.groupId).then(function (myGroup) {
    ...
    db.incident.findAll({
      ...
      where: {date: {[Op.between]: dateRange}},
      include: [includeUser]
...
function getExportedVideos(fromDate, toDate, group, filterGroupId, filterUserId, callback) {
  ...
  db.export.findAll({
    where: {createdAt: {[Op.between]: [fromDate, toDate]}}, include: [recorderQuery,
      exporterQuery]
...
function getTotalOfState(fromDate, toDate, myGroup, filterGroupId, filterUserId, state, callback) {
  ...
  db.history.findAll({
    where: {date: {[Op.between]: [fromDate, toDate]}, [Op.or]: [{previousState: state}, {nextState: state}]},
    order: [["userId", "asc"], ["date", "asc"]], include: userIncluded
    ...
    for (var i = 0; i < users.length; i++) {
      ...
      getDaysWithActivities(fromDate, toDate, user.id, function (userObj, user, days) {
        ...
        for (var i = 0; i < days.length; i++) {
          ...
          db.incident.findAll({
            where: {date: {[Op.between]: [initDate.toDate(), endDate.toDate()]}},
            include: [{
