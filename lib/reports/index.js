...
function loadIncidentForms(ret, group, fromDate, toDate, callback) {
  ...
  var query = 'SELECT gravity, COUNT(1) FROM "incidentForms" "form" ' +
    ...
    'WHERE "form"."date" BETWEEN :fromDate AND :toDate ';
...
function getVideosPlayed(dateRange, group, callback){
  ...
  db.sequelize.query(query, {
    ...
    for(var i = 0; i < results.length; i++){
      ...
      array.push({url: extras.videoName,startTime: extras.startTime ,date: h.date,  userName: h.name});
    }
    callback(array);
  });
}
...
function loadUsersData(ret, where, fromDate, toDate, group, callback) {
  ...
  db.user.findAll({
    ...
    var finished = _.after(users.length * 2 * (moment(toDate.toString()).diff(moment(fromDate.toString()), "days") + 1), callback);
    if (users.length === 0) {
      callback();
    }
