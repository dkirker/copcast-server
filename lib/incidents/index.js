...
app.post('/incidents', auth.ensureUser, function(req,res) {
  saveIncidentFlags(req, res);
});

app.post('/incidents/:user', auth.ensureUser, function(req,res) {
  db.user.find({where: {username: req.params.user}}).then(function(user) {
    saveIncidentFlags(req, res, user);
  });
});

var saveIncidentFlags = function(req, res, user) {
  ...
  var storeIncidentFlag = function (entry) {
  ...
  if ( !(req.body instanceof Array) ) {
    try {
      storeIncidentFlag(req.body);

      app.get('sockets').emit('users:incidentFlag', { id : u.id, name: u.name, username: u.username,
