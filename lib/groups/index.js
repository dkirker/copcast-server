...
app.delete('/groups/:id', auth.ensureAdminLevelOne, function(req, res) {
  db.group.findByPk(req.user.groupId).then(function(groupCurrent) {
    if (groupCurrent != null && groupCurrent.isAdmin === true) {

      db.group.findByPk(req.params.id).then(function (group) {
...
app.post('/groups/:id', auth.ensureAdminLevelOne, function(req, res) {
  db.group.findByPk(req.user.groupId).then(function(group){
    if (group != null && group.isAdmin === true ){
      ...
      db.group.findByPk(req.params.id).then(function(group){
        group.update(req.body).then(function(){
          res.send(group);
        }).catch(function(err) {
          res.status(422).send( err);
        });
...
app.post('/groups', auth.ensureAdminLevelOne, function(req, res) {
  db.group.findByPk(req.user.groupId).then(function(group){
    if (group != null && group.isAdmin === true ){
      var parsed = db.group.parseRequest(req.body);
      if (!parsed) {
        res.sendStatus(400);
      } else {
        db.group
