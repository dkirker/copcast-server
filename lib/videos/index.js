      ...
      db.video.create({id: uuid.v4(), date: dateRecorded.toISOString(), duration: Math.ceil(metadata.format.duration)})
        .then(function (video) {
          ...
          storage.createVideoStream(videoFile.path, video, metadata, function (streamError) {
            if (streamError) {
              video.destroy();
              console.log('STREAMER:', streamError.message);
...
app.post('/videos/:user', auth.ensureUser, upload.single('video'), function (req, res) {
    ...
    var dateRecorded = null;
    var videoFile = req.file;
    if (videoFile.originalname.indexOf('-')>0) { // for handling older versions of copcast
      if (videoFile.originalname.indexOf(':') > 0) {
        dateRecorded = moment(videoFile.originalname.replace('.mp4', ''), 'YYYY-MM-DDTHH:mm:ss.SSSZ');
      } else {
        dateRecorded = moment(videoFile.originalname.replace('.mp4', ''), 'YYYY-MM-DD_HH-mm-ss');
      }
    } else {
      dateRecorded = moment(new Buffer(videoFile.originalname.replace('.mp4', ''), 'base64').toString('ascii'), 'YYYY-MM-DD_HH-mm-ss');
    }
    ...
    db.user.find({where: {username: req.params.user}}).then(function(user){
      ...
      if ('rabbitmq' in config && config.rabbitmq.enabled) {
        ...
        rabbitmq.push({path: videoFile.path, user: user, date: dateRecorded.toISOString()});
        return res.sendStatus(201);
      } else {
        // otherwise resort to old direct ingestion
        console.log("Ingesting video");
        storage.ingestVideo(videoFile.path, user, dateRecorded.toISOString(), function (code, err) {
          if (err != null)
