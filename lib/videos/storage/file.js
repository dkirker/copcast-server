...
exports.compressAndRemoveDirectory = function(directory){
  var pathExport = zipBasePath + directory + '/'
  var zip = new AdmZip();
  zip.addLocalFolder(pathExport)
  zip.writeZip(zipBasePath + '/'+directory+'.zip');
...
function addExporterWatermark(video, tempfilename, filename, userRecorder, user, callback) {
  new FFmpeg({source: tempfilename }).on('end', function() {
      ...
      fontfile: fontsPath, text: 'video recorded by '+userRecorder.name + ' at '+
    moment(video.date).format('YY/MM/DD-HH-mm')+'. exported by '+user.name + ' at '+
    moment(video.date).format('YY/MM/DD-HH-mm'),
      fontsize: 10, fontcolor: 'white', boxcolor: 'black'} })
          .save(filename);
    }
    ...
    fs.createReadStream(basePath + video.encVideoPath()).on('error', function(err){
      ...
      fs.createReadStream(basePath + video.encAudioPath()).on('error', function(err){
        ...
        ffmpeg.on('end', function () {
          ...
          fs.unlinkSync(pathExport + video.audioPath())
          callback(err);
        }).addInput(pathExport + video.audioPath()).
