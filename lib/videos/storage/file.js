...
exports.compressDirectory = function(directory){
  var pathExport = zipBasePath + directory + '/'
  var zip = new Zip();
  zip.folder(pathExport).file(directory, 'hello there');
...
exports.exportVideoFile = function(directory, video, callback){
  var pathExport = zipBasePath + directory + '/'
  fs.mkdir(pathExport, 0777, true, function (err) {
    ...
    fs.createReadStream(basePath + video.encVideoPath()).on('error', function(err){
      ...
      fs.createReadStream(basePath + video.encAudioPath()).on('error', function(err){
        ...
        save(pathExport +video.userId + '/' + moment(video.date).format('YYY-MM-DD_hh-MM-ss') + '.mp4');
